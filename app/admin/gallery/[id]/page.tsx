...
export default async function EditGalleryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div style={{ maxWidth: '600px' }}>
      ...
      <form action={update}>
        <FormField label="Image URL" name="image_url" defaultValue={String(row.image_url ?? '')} />
        <FormField label="Storage Path" name="storage_path" defaultValue={String(row.storage_path ?? '')} />
